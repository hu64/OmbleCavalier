...
@pytest.mark.parametrize(
    "fen, description, expected_best_move",
    [
...
def test_puzzles(fen, description, expected_best_move):
    ...
    best_move = find_best_move_iterative(board, max_depth=6, total_time_remaining=1000)

    # Check if the best move matches expected (you can refine the check to allow UCI or Move object)
    assert str(best_move) == expected_best_move, (
        f"Expected {expected_best_move}, got {best_move}"
    )

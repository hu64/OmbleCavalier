...
def find_best_move(board, depth, total_time_remaining, prev_score=0):
    print(f"info string Finding best move at depth {depth} with total time remaining {total_time_remaining:.2f}s")
...
def find_best_move_iterative(board, max_depth, total_time_remaining):
    ...
    for depth in range(1, max_depth + 1):
        ...
        result = find_best_move(board, depth, total_time_remaining, prev_score)
        if result is None:
            break
